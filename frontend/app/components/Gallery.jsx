...
export default function Gallery() {
    ...
    return (
        <section className="relative py-24 bg-beige px-6 overflow-hidden min-h-screen" id="galerie">
            <div className="max-w-[1280px] mx-auto">
                {loading ? (
                    <div className="py-20 text-center italic opacity-50">Chargement de la galerie...</div>
                ) : (
                    <>
                        <motion.div 
                        ...
                        >
                            {themes.map((theme) => (
                                <motion.button
                                    key={theme.id}
                                    // Accessibilité : on ajoute le support clavier ici aussi
                            ...
                            <AnimatePresence mode="popLayout">
                                {visibleItems.map((item, idx) => (
                                    <motion.div 
                                        ...
                                        // Accessibilité : Ajout du focus clavier
                                        tabIndex="0"
                                        role="button"
                                        onKeyDown={(e) => e.key === "Enter" && setIndex(idx)}
                                        aria-label={`Agrandir ${item.title}`}
                                        className="group relative aspect-[4/5] overflow-hidden rounded-[30px] shadow-sm bg-white cursor-zoom-in focus:ring-2 focus:ring-primary focus:outline-none"
                                    >
                                        <img
                                            src={item.image_url}
                                            alt={item.title}
                        ...
                        {activeTheme?.items.length > visibleCount && (
                            <motion.div className="mt-16 text-center" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
                                <button 
                                    ...
                                    className="text-primary border-b border-primary/30 pb-1 hover:border-primary transition-all uppercase text-xs tracking-widest font-bold focus:outline-none focus:text-primary/70"
                                >
